...
// Delete removes a bin from persistent storage and from the in-memory list.
// Deleting an ID that is not stored locally is an idempotent operation.
func (list *BinList) Delete(id string) error {
	if err := list.storage.DeleteBin(id); err != nil {
		return fmt.Errorf("delete bin: %w", err)
	}

	for index := range list.bins {
		if list.bins[index].ID == id {
			list.bins = append(list.bins[:index], list.bins[index+1:]...)
			break
		}
	}
